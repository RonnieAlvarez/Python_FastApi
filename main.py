...
async def get_developers(id: str):
    try:
        ...
        if not developer:
            return JSONResponse(status_code=400,content={'message':'Desarrollador no encontrado'})    
...
async def update_developers(id: str):
    try:
        db = await connection()
        developer= await db.developers.find_one({'_id':ObjectId(id)})
        if not developer:
        ...
        return JSONResponse(status_code=201,content={'message':'Desarrollador Borrado'})
    except:
        return JSONResponse(status_code=500,content={'message':'Ha ocurrido un error'})

@app.put('/developers/{id}')
async def update_developers(data: Developer, id: str):
    try:
        db = await connection()
        developer= await db.developers.find_one({'_id':ObjectId(id)})
        if not developer:
        ...
        return JSONResponse(status_code=201,content={'message':'Desarrollador Actualizado'})
    except:
        return JSONResponse(status_code=500,content={'message':'Ha ocurrido un error'})